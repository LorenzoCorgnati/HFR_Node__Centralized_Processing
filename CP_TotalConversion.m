...
try
    for network_idx=1:numNetworks
        ...
        if(~strcmp(network_data{network_idx,network_idIndex},'HFR-MATROOS'))
            ...
            [TC_err, OpenDAPncData] = MATROOSreadOpenDAP(startDate,network_data(network_idx,:),network_columnNames);
            if(TC_err==0)
                ...
                for MtBC_idx=1:length(MATROOStoBeConvertedIndices)
                    % v2.1.2
                    [TC_err, network_data(network_idx,:), outputFilename,outputFilesize] = MATROOS2netCDF_v33(OpenDAPncData,MATROOStoBeConvertedIndices(MtBC_idx),network_data(network_idx,:),network_columnNames,station_data,station_columnNames);
                    ...
                    try
                        if((TC_err==0) && (exist('outputFilename','var') ~= 0))
                            ...
                            % Define a cell array that contains the data for insertion
                            addData = {outputFilename,network_data{network_idx,network_idIndex},DBtimestamp,DateTime,(datestr(now,'yyyy-mm-dd HH:MM:SS')),outputFilesize,0};
