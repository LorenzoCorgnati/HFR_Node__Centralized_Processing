...
try
    ...
    ncwrite(ncfile,'TIME_SEADATANET_QC',int8(sdnTime_QCflag));
    ncwrite(ncfile,'POSITION_SEADATANET_QC',sdnPosition_QCflag);
    ncwrite(ncfile,'DEPTH_SEADATANET_QC',int8(sdnDepth_QCflag));
...
if(T2C_err==0)
    disp(['[' datestr(now) '] - - ' 'tot2netCDF_v32.m successfully executed.']);
end
